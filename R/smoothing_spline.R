#' smoothing_spline: Fit noisy data with a tensioned interpolating spline.
#'
#' @return
#' @export
#'
...
#                     K <- varargin{k+1}
#                 } else if (strcmp(varargin{k}, 'S')){
#                     K <- varargin{k+1}+1
#                 } else if (strcmp(varargin{k}, 'T')){
#                     T <- varargin{k+1}
#                 } else if (strcmp(varargin{k}, 'lambda')){
#                     lambdaArgument <- varargin{k+1}
#                 } else if (strcmp(varargin{k}, 'mu')){
#                     mu <- varargin{k+1}
#                 } else if (strcmp(varargin{k}, 't_knot')){
#                     t_knot <- varargin{k+1}
#                 } else if (strcmp(varargin{k}, 'sigma')){
#                     sigma <- varargin{k+1}
#                     didOverrideSigma <- 1
#                 } else if (strcmp(varargin{k}, 'constraints')){
#                     constraints <- varargin{k+1}
#                 } else if (strcmp(varargin{k}, 'knot_dof')){
#                     if (ischar(varargin{k+1}) && strcmp(varargin{k+1}, 'auto')){
#                         shouldSetKnotDOFAutomatically <- 1
#                     } else if (isnumeric(varargin{k+1}) && varargin{k+1} >= 1){
#                         knot_dof <- varargin{k+1}
#                     } else {
#                         error('invalid option for (knot_dof. Set to a value > in  1 or auto.')){
#                     }
#                 }
#             }
#
...
#                        }
#                 }
#             } else if (isscalar(lambdaArgument)){
...
#                 }
#             } else if (!isempty(distribution.w)){
...
#             } else if (lambdaArgument == Lambda.optimalRangedIterated){
...
#             } else if (lambdaArgument == Lambda.fullTensionIterated){
...
#             t_T <- self.t_knot(self.K:1:(}-self.K+1))
#             t_T <- t_T(1:}-1) + diff(t_T)/2){
...
#                 }
#             } else if (!isempty(self.distribution.w)){
...
#             } else if (!isempty(self.variableCache.CmInv)){
...
#             tq <- linspace(self.t(1),self.t(length(t)),Q)'
...
#             }
#
#             while ((index < in  2 || (index > in  nLambdas-1 && isConstrained(index) ! in 1) )){
#                 if (index <= 2){
#                     # expand search below
#                     lambda0 <- lambdas(1)
...
#                     # expand search above
#                     lambda0 <- lambdas(length(lambdas))
...
#                     for (iLambda in (nLambdas+1):(nLambdas+n)){
#                         aSmoothingSpline.lambda <- lambdas(iLambda)
#                         err(iLambda) <-  functionOfSpline(aSmoothingSpline)
#                         isConstrained(iLambda) <- aSmoothingSpline.isConstrained
#                     }
...
#             epsilon <- 1e-15
#             for (iSpline in 1:length(tensionSplines)){
...
#             x_Q3 <- median(x(ceil(length(x)/2)+1:length(x)))
...
#
#             for (iAlpha in 1:length(alpha_outlier)){
...
#                     flag <- 1
#                 } else if (all(mod(dt_multiples,1.0) < 0.01)){
#                     flag <- 2
#                 } else {
#                     flag <- 0
#                 }
...
#             for (D in 0:(aSmoothingSpline.K-1)){
#
...
#                 if (nargin < 5){
#                     alpha <- 0.05
#                 } else if (!isnumeric(alpha) || numel(alpha) != 1 || alpha <= 0 || alpha >= 1){
...
#                 didnt <- didnt(1)
#                 if (numel(a) == 1, abad <- a; else abad <- a(didnt); }){
#                 if (numel(b) == 1, bbad <- b; else bbad <- b(didnt); }){
#                 if (numel(p) == 1, pbad <- p; else pbad <- p(didnt); }){
#                 warning('stats:gaminv:NoConvergence',...
#                     'GAMINV did not converge for (a  in ){#g, b = #g, p = #g.',...
#                     abad,bbad,pbad)
#             }
#
...
#                 warning(warn)
#             catch
#                 if (exist('warn','var'), warning(warn); }){
